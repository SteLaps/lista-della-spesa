...
TEST(TestOggetto, GirnoNonValido) {
    EXPECT_THROW(Oggetto latte("latte", Categoria::Latticini, 2, Data(50,4,2026)), std::invalid_argument);
}
...
TEST(TestOggetto, Creazione) {
    ...
    EXPECT_EQ(CocaCola.getDataDiScadenza(), "16/5/2026");
}
TEST(TestOggetto, setAcquistato) {
    Oggetto carota("carota", Categoria::Verdura, 8, Data(2, 6, 2026));
...
TEST(TestOggetto, getDataDiScadenza) {
    Data data(6, 5, 2026);
    Oggetto anguria("anguria", Categoria::Frutta, 2, data);
    EXPECT_EQ(anguria.getDataDiScadenza(), "6/5/2026");
}
TEST(TestOggetto, PiuOggetti) {
    ...
    EXPECT_EQ(arancia.getDataDiScadenza(), "4/4/2026");
    EXPECT_EQ(uva.getDataDiScadenza(), "9/3/2026");
}